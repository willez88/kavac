				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="row">
							...
							<div class="col-3"><strong>Tipo:</strong> {{ purchase_supplier_object }}</div>
...
export default{
	...
	data(){
		return{
			records:[],
			columns: ['name','warehouse_product.measurement_unit.name','technical_specifications', 'quantity'],
		}
	},
	created(){
		this.table_options.headings = {
				'name': 'Producto',
				'warehouse_product.measurement_unit.name': 'Unidad de Medida',
				'technical_specifications': 'Especificaciones tecnicas',
				'quantity': 'Cantidad',
			};
		this.table_options.columnsClasses = {
			'name'    : 'col-xs-4',
			'warehouse_product.measurement_unit.name': 'col-xs-2',
	...
	computed:{
		...
		purchase_supplier_object: function(){
			if (this.records.purchase_supplier_object) {
				...
				return type+' > '+this.records.purchase_supplier_object.name;
			}
		},
