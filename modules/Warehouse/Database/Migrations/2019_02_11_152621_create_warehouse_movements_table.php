        ...
        if (!Schema::hasTable('warehouse_movements')) {
            Schema::create('warehouse_movements', function (Blueprint $table) {
                ...
                $table->integer('type')->nullable()
                      ->comment('Tipo de movimiento (0 => Compras,1 => Interno, 2 => Venta)');

                $table->string('observation')->nullable()->comment('Observaciones sobre la operación');
                $table->boolean('complete')->default(false)
                      ->comment('Indica si la operación fue realizada o completada correctamente');

                /*
                 * Intituciones y Almacenes involucrados
                 */
                $table->integer('warehouse_inst_start_id')->nullable()
                      ->comment('Identificador único de la tabla (institución-almacen) de donde parten los artículos');
                $table->foreign('warehouse_inst_start_id')->references('id')->on('warehouse_institution_warehouses')
                      ->onDelete('restrict')->onUpdate('cascade');

                $table->integer('warehouse_inst_finish_id')->nullable()
                      ->comment('Identificador único de la tabla (institución-almacen) donde llegan los artículos');
                $table->foreign('warehouse_inst_finish_id')->references('id')->on('warehouse_institution_warehouses')
                      ->onDelete('restrict')->onUpdate('cascade');
